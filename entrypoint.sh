...
{
	git clone --single-branch --depth 1 --branch "$SOURCE_BRANCH" "$GIT_CMD_REPOSITORY" "$CLONE_DIR"
} || {
	echo "::error::Could not clone the destination repository. Command:"
	echo "::error::git clone --single-branch --branch $SOURCE_BRANCH $GIT_CMD_REPOSITORY $CLONE_DIR"
